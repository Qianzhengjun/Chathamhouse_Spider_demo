...
url = 'https://www.chathamhouse.org/publication/nato-npt-frameworks-caughley-afina'
...
# classification
classification = html.xpath('/html/body/section/section[1]/section/header/div/span[4]/a/text()')[0]
...
# content done
content = content + str.strip(html.xpath('//article/header/div/p/text()')[0]) + '\n'
uls = html.xpath('//article/section/div/ul/li')
for ul in uls:
    for i in range(0, 20):
        try:
            content = content + ul.xpath('./text()')[i]
            i = i + 1
        except:
            content = content +'\n'
            break
...
# print({title, country, classification, content, publish_date, author, '', '', title, content})
